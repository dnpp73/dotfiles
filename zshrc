...
export HISTORY_IGNORE="(cd|cd .*|pwd|l|l *|l[sal]|l[sal] *|xed .|git st|git ft|git pull|git lga|git df|git ci -m *|git br *|git add *|git ad|kill *|exit)" # 履歴に残さないコマンド。 l, ls, la, ll は履歴に残さない。
