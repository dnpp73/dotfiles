...
if [ "$(uname)" = 'Darwin' ]; then
    ...
    memory_inactive="$(echo   "${memory_result}" | head -n 4  | tail -n 1 | cut -d ' ' -f1-2)"
    memory_free="$(echo       "${memory_result}" | head -n 5  | tail -n 1 | cut -d ' ' -f1-2)"
    # memory_buffer="$(echo     "${memory_result}" | head -n 6  | tail -n 1 | cut -d ' ' -f1-2)"
    # memory_swap_cache="$(echo "${memory_result}" | head -n 7  | tail -n 1 | cut -d ' ' -f1-2)"
    # memory_total_swap="$(echo "${memory_result}" | head -n 8  | tail -n 1 | cut -d ' ' -f1-2)"
    memory_used_swap="$(echo  "${memory_result}" | head -n 9  | tail -n 1 | cut -d ' ' -f1-2)"
    memory_free_swap="$(echo  "${memory_result}" | head -n 10 | tail -n 1 | cut -d ' ' -f1-2)"
